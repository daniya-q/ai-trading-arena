...
let dailyGapPct = 0;
let gapCalcDate = "";
let orbHigh = {}; // "NIFTY" → ORB high (Orion's copy)
let orbLow = {}; // "NIFTY" → ORB low  (Orion's copy)
let orbSet = {};
...
function checkDailyReset() {
    ...
    if (tradeDateStr !== today) {
        ...
        for (const k of Object.keys(s6Orb))
            delete s6Orb[k];
...
function computeORB(index) {
    ...
    const istMid = Date.UTC(istD.getUTCFullYear(), istD.getUTCMonth(), istD.getUTCDate())
        - (5 * 60 + 30) * 60000;
    const orb915Ms = istMid + (9 * 60 + 15) * 60000; // 9:15:00 IST today, in UTC ms
...
function getATMOption(chain, type, premMin = 60, premMax = 70, spotPrice) {
...
async function runStrategy8() {
    ...
    const chain = getLatestChain("NIFTY");
    if (!chain) {
        console.log(`[S8] No option chain data`);
        return;
    }
    const fld = optType === "CE" ? "cePremium" : "pePremium";
    const allPrem = chain.rows.map(r => r[fld]).filter(p => p > 0).sort((a, b) => a - b);
    ...
    if (s8Quantity === 0) {
        console.log(`[S8] SIGNAL ${optType} — lot calc=0, skipping (capital=₹${Math.round(s8Capital).toLocaleString("en-IN")} premium=₹${option.premium})`);
        return;
    }
...
async function logVariantSignal(opts) {
    ...
    console.log(`${tag} ${opts.direction.toUpperCase()}${blocked}` +
...
let sAsymPrevFast = 0;
let sAsymPrevSlow = 0;
let sAsymPendingFlip = null;
async function runStrategyAsym() {
    if (!isMarketOpen())
        return;
    const mins = istMins();
    if (mins < 585 || mins >= 920)
    ...
    const emaDir = fastCurr > slowCurr ? "CE" : "PE";
    ...
    const currentType = hasPos ? openPos[0].type : null;
    ...
    const oppositeDir = currentType === "CE" ? "PE" : "CE";
...
let sConfirmPending = null;
async function runStrategyConfirm() {
    if (!isMarketOpen())
        return;
    const mins = istMins();
    if (mins < 585 || mins >= 920)
    ...
    if ((bearCross && currentType === "CE") || (bullCross && currentType === "PE")) {
        const optType = currentType === "CE" ? "PE" : "CE";
...
async function runStrategyDualTf() {
    if (!isMarketOpen())
        return;
    const mins = istMins();
    if (mins < 585 || mins >= 920)
...
async function runStrategy6() {
    ...
    if (!s6Orb["NIFTY"]) {
        ...
        if (!orb) {
            console.log(`[S6] ORB not available — 9:15 candle not found yet`);
            return;
        }
        s6Orb["NIFTY"] = orb;
...
const btcPeakPrices = {}; // posId → peak price (LONG: highest, SHORT: lowest)
const btcSlDists = {}; // posId → original SL distance in USD
...
async function updateBtcCapital(strategyId) {
    try {
        const { data } = await supabase
            .from("btc_strategy_positions")
...
async function backfillBtcSharpe() {
    const strategies = [
        ...
        for (const row of data) {
            ...
            if (row.partial_booked && config && slDist && slDist > 0) {
                ...
                for (let t = 0; t < config.tiers.length; t++) {
                    if (gainMultiple >= config.tiers[t].multiplier)
                        activeTierIdx = t;
                }
                if (activeTierIdx >= 0) {
                    ...
                    const shouldUpdate = row.side === "LONG"
                        ...
                        : (!row.trail_sl || newTrailSL < row.trail_sl);
                    if (shouldUpdate) {
                        const updates = { trail_sl: newTrailSL };
...
async function strategyBtcOrion() {
    ...
    const blockStartMs = Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), blockHour, 0, 0, 0);
    ...
    if (minsIntoBlock < 15) {
        ...
        const orbCandles = candles30s.filter(c => c.time >= blockStartMs && c.time < blockStartMs + 15 * 60000);
    ...
    if (total >= 10) {
        console.log(`[BTC:orion] Daily limit reached (${total}/10)`);
        return;
    }
    if (openPos && openPos.length > 0)
...
function calcMetrics(trades, allocated) {
    if (trades.length === 0) {
...
app.get("/api/strategy-metrics", async (req, res) => {
    const strategy = String(req.query.strategy ?? "").toLowerCase();
    if (!strategy) {
        res.status(400).json({ error: "strategy param required" });
        return;
    }
    ...
    res.json(calcMetrics((tRes.data ?? []), allocated));
});
...
app.get("/api/correlation", async (req, res) => {
    ...
    const STRATS = (stratRows ?? []).map((r) => r.id);
    if (STRATS.length === 0) {
        res.json({ strategies: [], matrix: [] });
        return;
    }
    ...
    const matrix = STRATS.map((si, i) => STRATS.map((sj, j) => {
        if (i === j)
            return 1;
